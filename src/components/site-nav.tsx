      ...
      <div className="glass-card mx-3 mt-3 rounded-2xl px-4 py-3 md:mx-6 md:px-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-7 text-sm font-medium text-foreground/80 lg:flex">
            {NAV.map((n) => (
              <Link key={n.label} to={n.to} className="transition-colors hover:text-royal" activeProps={{ className: "text-royal" }}>
                {n.label}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-2">
            <Link to="/account" aria-label="Account" className="grid h-10 w-10 place-items-center rounded-full text-foreground/80 transition-colors hover:bg-cream-deep">
              <User className="h-5 w-5" />
            </Link>
            <Link to="/cart" aria-label="Cart" className="relative grid h-10 w-10 place-items-center rounded-full bg-ink text-cream transition-transform hover:scale-105">
